...
import {
  ...
  FurnitureValueSales,
} from './FurnitureValueGridContainer.types';
import {
  FurnitureValueGridContainerElement,
  FurnitureValueGridContainerStatElement,
} from './FurnitureValueGridContainer.styled';
import {
  useFurniturePurchaseLogOverviewFetchOne,
...
const LAST_WEEKS_START_DATE = DayJS(THIS_WEEKS_END_DATE)
  .subtract(1, 'week')
  .unix();
const LAST_WEEKS_END_DATE = THIS_WEEKS_END_DATE;

export function FurnitureValueGridContainer({
  furniture,
}: FurnitureValueGridContainerProps) {
  ...
  const onFetchFurniValue = async () => {
    ...
    const thisWeeksSells = await fetchTotalSells.fetch({
      ...
      endDate: THIS_WEEKS_END_DATE,
    });
    const lastWeeksSells = await fetchTotalSells.fetch({
      furnitureID: furniture.id,
      startDate: LAST_WEEKS_START_DATE,
      endDate: LAST_WEEKS_END_DATE,
    });
    const difference = Number(thisWeeksSells - lastWeeksSells);
    const differencePercent = Number(thisWeeksSells / lastWeeksSells).toFixed(
      2
    );
    setSellsChange({
      ...
      <FurnitureValueGridContainerElement>
        ...
        <GridLarge style={{height: 150}}>
          <FurnitureValueGridContainerStatElement>
            ...
            {Number(
              fetchPurchaseLogOverview.data?.averageCostCredits ?? 0
            ).toLocaleString()}
          </FurnitureValueGridContainerStatElement>
          <FurnitureValueGridContainerStatElement>
            <img src="/img/diamonds.svg" loading="lazy" />
            {Number(
              fetchPurchaseLogOverview.data?.averageCostPoints ?? 0
            ).toLocaleString()}
          </FurnitureValueGridContainerStatElement>
          <FurnitureValueGridContainerStatElement>
          ...
          <FurnitureValueGridContainerStatElement
            style={{
              color: sellsChange?.difference
                ? sellsChange.difference > 0
                  ? 'green'
                  : 'red'
                : 'initial',
            }}
