...
export function HighScoresByRPStatsGrid({
  children,
  orderBy,
}: HighScoresByRPStatsGridProps) {
  const [page, setPage] = useState(0);
  const rpStatsFetchMany = useRPStatsFetchMany();

  const canGoDown = page > 0;
  const canGoUp =
    (rpStatsFetchMany?.data?.length ?? 0) >= HIGH_SCORES_PAGE_SIZE;
  ...
  async function refresh() {
    await rpStatsFetchMany.fetch({
      orderBy,
      skip: page & HIGH_SCORES_PAGE_SIZE,
      limit: HIGH_SCORES_PAGE_SIZE,
    });
  }
