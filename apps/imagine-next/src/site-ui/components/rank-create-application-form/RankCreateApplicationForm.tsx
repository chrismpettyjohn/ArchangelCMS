...
import {
  useStaffApplicationCreate,
  useStaffApplicationFetchMany,
} from '@imagine-cms/client';

export function RankCreateApplicationForm({
  rank,
}: RankCreateApplicationFormProps) {
  ...
  const isLoading =
    createStaffApplication.loading || fetchStaffApplication.loading;

  const hasAlreadyApplied =
    (fetchStaffApplication?.data?.length ?? 0) > 0 || isLoading;
  ...
  const onFetchApplication = async () => {
    ...
    const response = await fetchStaffApplication.fetch({
      rankIDs: [rank.id],
      userIDs: [session.id],
    });
  ...
  return (
    <Form disabled={isDisabled} onSubmit={onSubmitApplication}>
      ...
      />
      {hasAlreadyApplied && (
        <p style={{color: 'red'}}>You already applied for this role!</p>
      )}
      <div style={{display: 'flex', flex: 1, justifyContent: 'flex-end'}}>
        <ButtonSuccess disabled={isDisabled} type="submit">
          <i
            className={
              isLoading ? 'fa fa-spinner fa-spin' : 'fa fa-file-contract'
            }
            style={{marginRight: 8}}
          />{' '}
          Submit Application
