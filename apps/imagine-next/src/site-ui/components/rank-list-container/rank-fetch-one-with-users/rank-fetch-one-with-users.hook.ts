...
import {
  RANK_FETCH_ONE_WITH_USERS_QUERY,
  RankFetchOneWithUsersQueryResponse,
  RankFetchOneWithUsersQueryVariables,
  RankWithUsersFragment,
} from './rank-fetch-one-with-users.query';
...
export function useRankFetchOneWithUsers(): UseRankFetchOneResponse {
  const [getRank, {loading, error, data}] = useLazyQuery<
    RankFetchOneWithUsersQueryResponse,
    RankFetchOneWithUsersQueryVariables
  >(RANK_FETCH_ONE_WITH_USERS_QUERY);

  const onFetchRank = async (
    filter: RankFilterOneInput
  ): Promise<RankWithUsersFragment> => {
    const matchingRank = await getRank({
