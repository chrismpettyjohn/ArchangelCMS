...
import {
  ...
  HealthProgress,
  HungerProgress,
} from './RPStatsGridContainer.styled';
  ...
  const [
    ...
    return [
      ...
      (fetchRPStats.data?.hungerMax ?? 0) -
        (fetchRPStats.data?.hungerCurrent ?? 0),
      fetchRPStats.data?.hungerMax ?? 0,
      (fetchRPStats.data?.armorMax ?? 0) -
        (fetchRPStats.data?.armorCurrent ?? 0),
      fetchRPStats.data?.armorMax ?? 0,
    ];
  }, [fetchRPStats.data]);

  const [healthPercent, energyPercent, hungerPercent, armorPercent] =
    useMemo(() => {
      return [
    ...
    <GridLarge>
      <div style={{width: '100%'}}>
        <div style={{display: 'flex', justifyContent: 'space-between'}}>
          <label>
            <i className="fa fa-heart" style={{marginRight: 4}} /> Health
          </label>
        ...
        >
          {healthCurrent === 0 && (
            <span style={{color: 'red'}}>Visit a doctor</span>
          )}
        </HealthProgress>
      </div>
      <div>
        <div style={{display: 'flex', justifyContent: 'space-between'}}>
          <label>
            <i className="fa fa-bolt" style={{marginRight: 4}} /> Energy
          </label>
        ...
        >
          {energyCurrent === 0 && (
            <span style={{color: 'red'}}>You have asthma</span>
          )}
        </EnergyProgress>
      </div>
      <div>
        <div style={{display: 'flex', justifyContent: 'space-between'}}>
          <label>
            <i className="fa fa-utensils" style={{marginRight: 4}} /> Hunger
          </label>
        ...
        >
          {hungerCurrent === 0 && (
            <span style={{color: 'red'}}>You are starving.</span>
          )}
        </HungerProgress>
      </div>
      <div>
        <div style={{display: 'flex', justifyContent: 'space-between'}}>
          <label>
            <i className="fa fa-shield" style={{marginRight: 4}} /> Armor
          </label>
        ...
        >
          {armorMax === 0 && (
            <span style={{color: 'red'}}>You don't have any armor.</span>
          )}
        </ArmorProgress>
      </div>
    </GridLarge>
