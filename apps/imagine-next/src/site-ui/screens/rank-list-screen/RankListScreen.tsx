...
const RANK_MOCK_CONTAINERS = Array(4)
  .fill(null)
  .map((_, i) => <RankListContainerMock key={`rank_mock_${i}`} />);
...
export function RankListScreen() {
  ...
  if (loading) {
    return <GridLarge>{RANK_MOCK_CONTAINERS}</GridLarge>;
  }

  return (
    <GridLarge>
      {data?.map(_ => <RankListContainer key={`rank_${_.id}`} rank={_} />)}
    </GridLarge>
  );
}
